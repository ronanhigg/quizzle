...
class Upload_ImageException extends Exception {}

class Upload_ImageNoFileException extends Upload_ImageException {}
...
class Upload_Image
{
    ...
    public function process($field_name)
    {
        try {
            ...
            throw new Upload_ImageNoFileException($e->getMessage());
        }
        ...
        if ( ! $is_successful) {
            throw new Upload_ImageException('An error occurred while uploading the file to AWS S3.');
        }
    ...
    public function remove($url)
    {
        ...
        if ( ! $is_deleted) {
            throw new Upload_ImageException('Storyboard image was not deleted from AWS S3.');
        }
    }
    ...
    public function delete_file($file_path)
    {
        ...
        if ( ! $is_uploaded_file_removed) {
            throw new Upload_ImageException('Could not remove uploaded file from local server.');
        }
    }
}
